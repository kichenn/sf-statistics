...
public class AhoCorasickDoubleArrayTrie<V> implements Serializable
{
    ...
    /**
     * test text match any keyword
     * @param text The text
     * @return true if text match any keyword
     */
    public boolean testMatch(String text)
    {
        int currentState = 0;
     ...
     * @return a list of outputs
     */
    public V getFirstMatch(String text)
    {
        int position = 1;
        int currentState = 0;
        V first = null;
        for (int i = 0; i < text.length(); ++i)
        {
            ...
            {
            	int hit = hitArray[0];
            	Hit<V> firstHit = new Hit<V>(position - l[hit], position, v[hit]);
            	first = firstHit.value;
            	break;// break iteration
            }
        }
        return first;
